...
declare module 'amqplib' {
  ...
  declare namespace a {
    ...
    interface Connection extends event.EventEmitter {
    ...
    declare type ConsumerHandler = (msg: Message) => void;
    ...
    interface SendOptions {
      ...
      /**
       * Either 1 or falsey, meaning non-persistent; or, 2 or truthy, meaning persistent. That's just obscure though. Use the option persistent instead.
       */
    ...
    interface ConsumeOptions {
      ...
      /**
       * in theory, if true then the broker won't deliver messages to the consumer if they were also published on this connection; RabbitMQ doesn't implement it though, and will ignore it. Defaults to false.
       */
      noLocal?: boolean;

      /**
       * if true, the broker won't expect an acknowledgement of messages delivered to this consumer; i.e., it will dequeue messages as soon as they've been sent down the wire. Defaults to false (i.e., you will be expected to acknowledge messages).
       */
      noAck?: boolean;

      /**
       * if true, the broker won't let anyone else consume from this queue; if there already is a consumer, there goes your channel (so usually only useful if you've made a 'private' queue by letting the server choose its name).
       */
      exclusive?: boolean;

      /**
       *  gives a priority to the consumer; higher priority consumers get messages in preference to lower priority consumers. See this RabbitMQ extension's documentation
       */
      priority?: number;
    ...
    interface Channel extends event.EventEmitter {
      close(): Promise<void>;

      assertQueue(
        queue?: string,
        options?: Partial<AssertQueueOptions>
      ): Promise<void>;

      checkQueue(queue: string): Promise<void>;
