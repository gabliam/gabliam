...
import { ExpressConfigMetadata, ExpressPluginConfig, ControllerMetadata, ControllerMethodMetadata } from './interfaces';
...
export class ExpressPlugin implements coreInterfaces.GabliamPlugin {
  ...
  private buildControllers(container: inversifyInterfaces.Container, registry: Registry) {
    const restConfig = container.get<ExpressPluginConfig>(EXPRESS_PLUGIN_CONFIG);
