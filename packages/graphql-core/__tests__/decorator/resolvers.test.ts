...
[QueryResolver, MutationResolver, SubscriptionResolver, MapResolver].forEach(
  decorator => {
    describe(`${decorator.name} TU`, () => {
      test(`should add ${
        decorator.name
      } metadata to a class when decorated with @${decorator.name}()`, () => {
        @GraphqlController()
        class TestBean {
          @decorator()
          test1(): any {
      ...
      test(`should add ${
        decorator.name
      } metadata to a class when decorated with @${
        decorator.name
      }({graphqlFiles: absolute})`, () => {
        @GraphqlController()
        class TestBean {
          @decorator({
